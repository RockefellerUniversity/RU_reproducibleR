---
title: "Using containers with Docker<html><div style='float:left'></div><hr color='#EB811B' size=1px width=796px></html>"
...
* [Reproducible_R](https://rockefelleruniversity.github.io/RU_reproducibleR/)

Or can be downloaded as a zip archive from here.

* [Download zip](https://github.com/RockefellerUniversity/RU_reproducibleR/archive/master.zip)
...
The Docker client, typically the command line interface, communicates with the Docker daemon based on [user commands.](https://docs.docker.com/engine/reference/commandline/cli/)
...
Once the image is on our system, we can launch a container with the ['docker container run' command](https://docs.docker.com/engine/reference/commandline/run/).
...
These can be identified in the log for the build command and manually added to the *apt-get* command, or alternatively, dependencies for a specific R or Python package can be found using the [Posit package manager](https://packagemanager.posit.co). This resource can also be used to install only those libraries that are necessary, which can help to limit the size of the image.  
...
Python also has [images available](https://hub.docker.com/_/python) on Docker Hub and we can make a Dockerfile that has a specific version of Python and any other packages we want available in that environment.
...
Open a new R project and then open the 'test_renv.R' script in the 'r_course/data/renv_docker' folder within the course materials. 

```{r, echo=FALSE, out.width="80%", fig.align='left'}
knitr::include_graphics("imgs/test_renv_script.png")
```

After loading these libraries from the script in the same folder as the project, initialize the renv.
```{r, eval=F}
# initialize renv
renv::init()
```
...
Apptainer was designed to play nice with Docker and the [Apptainer manual](https://apptainer.org/docs/user/main/docker_and_oci.html) goes into detail about how to use Apptainer with Docker images. 
